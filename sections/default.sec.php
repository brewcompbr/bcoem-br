...
$print_page_link = "<p><span class='icon'><img src='".$base_url."images/printer.png' border='0' alt='Print' title='Print' /></span><a id='modal_window_link' class='data' href='".$base_url."output/print.php?section=".$section."&amp;action=print' title='Print'>Imprima essa página</a></p>";
$competition_logo = "<img src='".$base_url."user_images/".$_SESSION['contestLogo']."' width='".$_SESSION['prefsCompLogoSize']."' style='float:right; padding: 5px 0 5px 5px' alt='Competition Logo' title='Competition Logo' />";
$page_info = "";
$page_info1 = "";
$header1_1 = "";
...
$primary_page_info = "";
$primary_page_info .= sprintf("<p>Obrigado pelo seu interesse no <strong>%s</strong> organizado pela ",$_SESSION['contestName']);
...
$primary_page_info .= sprintf(".  Por favor leia o  <a href='%s'>regulamento do concurso</a>.</p>",build_public_url("rules","default","default",$sef,$base_url));
...
if ((judging_date_return() == 0) && ($registration_open == "2")) {
	...
	if (!isset($_SESSION['loginUsername'])) {
		$page_info .= "<p>Você só precisa se cadastrar uma vez e pode retornar ao site para adicionar suas amostras ou editar as amostras já cadastradas.";
		if ($_SESSION['prefsPaypal'] == "Y") $page_info .= " Você pode pagar a taxa de suas amostras online.";
		$page_info .= "</p>";
	}
	
	if (($_SESSION['prefsSponsors'] == "Y") && ($totalRows_sponsors > 0)) {
		
		$header1_1 .= "<h2>Patrocinadores</h2>";
		$page_info1 .= sprintf("<p>%s se orgulha de ter como patrocinadores ",$_SESSION['contestHost']);
        ...
        $page_info1 .= sprintf(" para o %s.",$_SESSION['contestName']);
